...
class FpurchaseOrder extends FcommunicationDb {
    public function store(string $table, keyValues $order) {
        $fCreditCard = new FcreditCard();
        if(!$fCreditCard->exist("CreditCard", $order->getPaymentMethod() )) {
            $fCreditCard->store("CreditCard", $order->getPaymentMethod());
        }
        $fAddress = new Faddress();
        if(!$fAddress->exist("Address", $order->getShippingAddress() )) {
